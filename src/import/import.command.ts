...
@Command({
    name: 'import',
    description:
        'Import latest dataset of private health insurance data from data.gov.au',
})
export class ImportCommand extends CommandRunner {
    ...
    async run(
        ...
        else if (options.cache) await this.importService.cache();
        else await this.importService.run(options.force);
    }
    ...
    async displayStatus(
        ...
        const lastCheck = await this.systemService.get(
            'IMPORT',
            'LASTCHECK',
            'none',
        );
        const lastRun = await this.systemService.get(
            'IMPORT',
            'LASTRUN',
            'none',
        );
        const lastFile = await this.systemService.get(
            'IMPORT',
            lastRun,
            'none',
        );
    ...
    @Option({
        flags: '-f, --force [force]',
        description:
            'Force importing latest dataset, even if it has already been imported',
    })
