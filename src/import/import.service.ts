...
const URL =
    'https://data.gov.au/data/api/3/action/package_show?id=private-health-insurance';
...
export class ImportService {
    ...
    private async unzip(
        ...
        xt.on('data', (data: any) => {
            ...
            if (++count % 2500 === 0)
                this.logger.log(
                    `Processing '${filename}', loaded ${count} ${tag} elements.`,
                );
        });
        // stream zip file to xtreamer..
        await pipeline(zip.file(filename).nodeStream(), xt);
        this.logger.log(
            `Processed '${filename}', loaded ${count} ${tag} elements.`,
        );
    }
    ...
    async initAncillaryTables() {
        ...
        await this.productsService.createHealthService(
            'ACU',
            'G',
            'None',
            'Acupuncture',
    ...
    async run(force: boolean) {
        ...
        if (!response.ok) {
            this.logger.error(
                'Error fetching data package from data.gov.au:' +
                    response.statusText,
            );
            return;
        }
        ...
        const lastRunTime = await this.systemService.get(
            'IMPORT',
            'LASTRUN',
            new Date(0).toString(),
        );
        const lastFileLoaded = await this.systemService.get(
            'IMPORT',
            lastRunTime,
            'no run found',
        );
        ...
        if (!response.ok) {
            this.logger.error(
                'Error fetching data resource from from data.gov.au:' +
                    response.statusText,
            );
            return;
        }
        ...
        const files = ['fund file goes here'];
        zip.forEach((relativePath, zipEntry) => {
            if (zipEntry.name.startsWith('Funds ')) files[0] = zipEntry.name;
            if (
                zipEntry.name.startsWith('Combined Open ') ||
        ...
        let fundCompressMode = (process.env['FUND_XML_CACHE'] ||
            'compressed') as CacheMode;
        ...
        this.cacheService.writeCache(
            'funds/xml',
            fundCompressMode,
            zip.files[files[0]].nodeStream(),
        );

        // Process each of the files we are interested in.
        await this.unzip(zip, files[0], 'Fund', (xml: any) => {
            this.fundsService.createFromXML(xml);
        });
        for (let i = 1; i < files.length; i++)
            await this.unzip(zip, files[i], 'Product', (xml: any) => {
                this.productLoadService.createFromXML(xml, thisRunTime);
            });
