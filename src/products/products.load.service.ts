...
export class ProductsLoadService {
    ...
    async createFromXML(xml: any, timeStamp: Date): Promise<Product> {
        ...
        if (unicodeErr >= 0)
            this.logger.warn(
                `Unicode character detected in ${fundCode}/${prodCode}`,
            );
        ...
        product.excess =
            excessValues.length > 0 ? Math.min(...excessValues) : 0;

        product.hospitalTier = 'None';
        product.services = '';
        product.isCorporate =
            prodNode
                .getElementsByTagName('Corporate')[0]
                .getAttribute('IsCorporate') === 'true';
        product.brands = null;
        product.timeStamp = timeStamp;

        for (const brand of prodNode.getElementsByTagName('Brands')[0]
            .childNodes) {
            if (product.brands === null) product.brands = brand.textContent;
            else product.brands += `;${brand.textContent}`;
        }
        product.fundBrandCode = product.brands
            ? product.brands
            : product.fundCode;

        const elem = prodNode.getElementsByTagName('OnlyAvailableWith')[0]
        ...
        for (const serviceNode of prodNode.getElementsByTagName(
            ...
            if (covered !== 'NotCovered') {
                const key = await this.productsService.mapHealthService(
                    'H',
                    title,
                );
